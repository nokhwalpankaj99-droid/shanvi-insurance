...
function submitAccountingExam(){
  ...
  panel.innerHTML=`<div class="exam-score">${score}/${ACCOUNTING_EXAM.length} · ${pct}%</div><h3>${pass?'Congratulations — Exam Passed':'Exam Not Passed'}</h3><p>${pass?'You have met the 70% passing requirement. You can now generate your Shree Balaji Accounting course-completion certificate.':'The passing requirement is 70%. You can restart the exam and try again.'}</p><div class="exam-review">${ACCOUNTING_EXAM.map((q,i)=>{const ok=examState.answers[i]===q.a;return `<div class="daily-review-row"><b>Q${i+1}</b><span>${ok?'✓ Correct':'✕ Wrong'}</span><small>Your answer: ${examState.answers[i]===null?'Not Attempted':escapeHTML(q.o[examState.answers[i]])} · Correct: ${escapeHTML(q.o[q.a])}</small></div>`}).join('')}</div><div class="certificate-action"><button class="sts-btn primary" onclick="${pass?'showCertificateForm()':'restartAccountingExam()'}">${pass?'Generate Certificate':'Retake Exam'}</button><a class="sts-btn outline" href="#academy">Back to Academy</a></div>${pass?'<div id="certificateArea"></div>':''}`;
}
