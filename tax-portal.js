...
function loadDailyQuiz(){updateDailyQuizStats();}
...
/* Shree Balaji Accounting - Accounting Certification Exam */
...
function submitAccountingExam(){
  ...
  panel.innerHTML=`<div class="exam-score">${score}/${ACCOUNTING_EXAM.length} · ${pct}%</div><h3>${pass?'Congratulations — Exam Passed':'Exam Not Passed'}</h3><p>${pass?'You have met the 70% passing requirement. You can now generate your Shree Balaji Accounting course-completion certificate.':'The passing requirement is 70%. You can restart the exam and try again.'}</p><div class="certificate-action"><button class="sts-btn primary" onclick="${pass?'showCertificateForm()':'restartAccountingExam()'}">${pass?'Generate Certificate':'Retake Exam'}</button><a class="sts-btn outline" href="#academy">Back to Academy</a></div>${pass?'<div id="certificateArea"></div>':''}`;
}
...
function renderCertificate(){
 ...
 area.innerHTML=`<div class="certificate-preview" id="printCertificate"><div class="cert-org">SHREE BALAJI ACCOUNTING</div><h2>Certificate of Completion</h2><p>This certificate is proudly presented to</p><div class="cert-name">${escapeHTML(name)}</div><p>for successfully passing the Accounting & GST Certification Examination conducted through<br><b>Shree Balaji Accounting</b> in association with <b>Smart Tax Solution</b>.</p><p><b>Examination Score: ${Math.round((examState.answers.filter((v,i)=>v===ACCOUNTING_EXAM[i].a).length/ACCOUNTING_EXAM.length)*100)}%</b><br>Certificate ID: ${id}<br>Date: ${date}</p><div class="cert-footer"><span class="cert-founder">Adv. Pankaj Nokhwal<br>Founder, Smart Tax Solution</span><span>Shree Balaji Accounting<br>Course Completion Certificate</span></div></div><button class="sts-btn primary" style="margin-top:12px" onclick="printCertificate()">Print / Save Certificate</button>`;
}
function printCertificate(){const el=document.getElementById('printCertificate');if(!el)return;const w=window.open('','_blank','width=1000,height=750');w.document.write(`<html><head><title>Shree Balaji Accounting Certificate</title><style>body{font-family:Arial,sans-serif;padding:30px}.certificate-preview{border:7px double #b18a4d;padding:70px 40px;text-align:center;min-height:520px}.cert-org{letter-spacing:2px;font-weight:900}.certificate-preview h2{font-family:Georgia,serif;font-size:36px}.cert-name{font-family:Georgia,serif;font-size:30px;font-weight:700;border-bottom:1px solid #b18a4d;display:inline-block;padding:0 25px 10px}.certificate-preview p{line-height:1.7}.cert-footer{display:flex;justify-content:space-between;margin-top:45px;font-size:12px}</style></head><body>${el.outerHTML}</body></html>`);w.document.close();w.focus();w.print();}

function openWhatsAppEnquiry(service){
  const text='Hello Smart Tax Solution, I need '+service+' service. Please share details and documents required.';
  window.open('https://wa.me/919664029638?text='+encodeURIComponent(text),'_blank','noopener');
}
